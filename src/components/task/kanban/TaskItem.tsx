...
const TaskItem: React.FC<TaskItemProps> = ({
  ...
  // Dodatkowe funkcje obsługi HTML5 Drag and Drop API 
  // (uzupełnienie dla systemu react-dnd, aby wspierać upuszczanie na puste kolumny)
  const handleDragStart = (e: React.DragEvent<HTMLDivElement>) => {
    // Zapisz ID zadania w danych transferu, aby pusta kolumna mogła je odczytać
    e.dataTransfer.setData("text/plain", task.id);
    e.dataTransfer.effectAllowed = "move";
    
    // Możemy dodać dodatkowe dane jeśli potrzeba
    if (ref.current) {
      // Ustawienie miniatury przeciąganego elementu (opcjonalne)
      // e.dataTransfer.setDragImage(ref.current, 20, 20);
      
      // Dodaj klasę dla stylu podczas przeciągania
      ref.current.classList.add('opacity-50');
    }
  };

  const handleDragEnd = (e: React.DragEvent<HTMLDivElement>) => {
    // Usuń klasę stylu po zakończeniu przeciągania
    if (ref.current) {
  ...
  return (
    <>
      <div
        ref={ref}
        style={{ opacity }}
        className="relative p-5 bg-white border border-gray-200 task rounded-xl shadow-theme-sm dark:border-gray-800 dark:bg-white/5 cursor-move"
      ...
      >
        <div className="space-y-4">
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              {/* Dropdown Menu - dodajemy wyraźny przycisk */}
              <div className="relative">
                <button 
                  ...
                  aria-label="Menu opcji projektu"
                >
                ...
                <Dropdown
                  ...
                  className="absolute right-0 top-full z-40 w-[160px] space-y-1 rounded-2xl border border-gray-200 bg-white p-2 shadow-theme-md dark:border-gray-800 dark:bg-gray-dark"
                >
                  <DropdownItem
                    onItemClick={() => {
        ...
        {/* Assigned to */}
        <div className="absolute top-5 right-12 h-6 w-full max-w-6 overflow-hidden rounded-full border-[0.5px] border-gray-200 dark:border-gray-800">
